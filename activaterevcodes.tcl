       ...
       if { [lindex $r2 0] == "#pragma" } {
          ...
          if { $curtopic != "command" && $curtopic != "logevent" } {
            ...
            if { [lindex [lindex $rec 0] 0] != "const" } {
              ...
              if { [lindex [lindex $rec 0] 0] == "unsigned" } {
                set item [string trim [lindex $rec 2] "\[\];"]
              }
...
global SAL_WORK_DIR REVCODE SYSDIC
  ...
  if { [info exists SYSDIC($subsys,keyedID)] } {
    puts $fout "	#pragma keylist ackcmd_[set revcode] [set subsys]ID"
  } else {
    puts $fout "	#pragma keylist ackcmd_[set revcode]"
  }
  puts $fout "\};"
  close $fin
  close $fout
