#ifndef ORG_LSST_SAL_salcommand_READER_HPP_
#define ORG_LSST_SAL_salcommand_READER_HPP_
 ...
 */
template<typename T>
class sal::salcommand::Reader: public sal::salcommand::Runner<T> {
public:
   virtual ~Reader() {
   }
...
public:
         ...
         const dds::topic::Topic<T>& topic, const Params& params)
   {
      dds::sub::qos::SubscriberQos sqos =
            dp.default_subscriber_qos() << Partition("salcommand");
      ...
      std::vector< Sample<SALTopic_commandType> > data(max_samples);


      // AnyTopic work fine
      AnyTopic at = topic;
      dds::topic::Topic<SALTopic_commandType> xt = at.get<SALTopic_commandType>();
      ...
      DataReader<SALTopic_commandType> dr(sub, topic, qos_provider.datareader_qos());
      ...
      DataReader<SALTopic_commandType> xdr = adr.get<SALTopic_commandType>();
      ...
      LoanedSamples<SALTopic_commandType> loaned_samples = dr.read();
      ...
      for (uint32_t i = 0; i < samples; ++i) {
         ...
         std::cout << "==== Read  ==== \n";
         std::for_each(data.begin(), data.begin() + rs, printSALTopic_commandSample);
         std::cout << std::endl;
         usleep(sleep_time);

         LoanedSamples<SALTopic_commandType> loaned_samples =
               dr.selector()
                        .filter_state(params.data_state)
                        .read();

         std::cout << "==== Loaned Samples Read ==== \n";
         std::for_each(loaned_samples.begin(), loaned_samples.end(), printSALTopic_commandSample);
         std::cout << std::endl;
         usleep(sleep_time);
      }
   }
...
#endif /* ORG_LSST_SAL_salcommand_READER_HPP_ */
