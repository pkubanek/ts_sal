...
#include "telemetry_readerT.hpp"
#include "telemetry_writerT.hpp"

namespace po = boost::program_options;
typedef sal::saltest::Config Config_t;
typedef sal::saltest::Params Params_t;

Config_t init(int argc, char* argv[]) {
  // The configuration is already initialized with
  // default parameters.
  Config_t config;
  config.runner.reset(new sal::saltest::Reader<SALTopicType>());
  ...
  try {
    ...
    if (vm.count("writer")) {
      config.runner.reset(new sal::saltest::Writer<SALTopicType>());
    }
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    dds::topic::Topic<SALTopicType> saltopic(dp, config.params.name);
