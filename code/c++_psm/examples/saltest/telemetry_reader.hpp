...
public:
         ...
         const dds::topic::Topic<T>& topic, const Params& params)
   {
      ...
      std::vector< Sample<SALTopicType> > data(max_samples);


      // AnyTopic work fine
      AnyTopic at = topic;
      dds::topic::Topic<SALTopicType> xt = at.get<SALTopicType>();
      ...
      // DataReader<SALTopicType> dr(sub, topic, qos_provider.datareader_qos());
      ...
      // DataReader<SALTopicType> xdr = adr.get<SALTopicType>();
      ...
      LoanedSamples<SALTopicType> loaned_samples = dr.read();
