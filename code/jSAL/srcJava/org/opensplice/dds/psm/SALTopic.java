...
import org.lsst.sal.dds.dcps.TypeSupportImpl;
...
public class SALTopic<TYPE> implements Topic<TYPE> {
	...
	private final SALDomainParticipant participant;
	private final String topicName;
	private final Class<TYPE> type;
	private DDS.Topic topic= null;
	private TopicQos qos = null;
        private String private_revCode;
	private double private_sndStamp;
	private double private_rcvStamp;
	private long private_origin;
	
	public DDS.Topic getTopic() {
		return topic;
	}
	
	public SALTopic(SALDomainParticipant theparticipant, String thetopicName, Class<TYPE> thetype, TopicQos theqos) {
	...
	protected void createTopic() {
		...
		topic = participant.getSALParticipant().find_topic(topicName, new Duration_t(0, 50000000));
		if (null != topic) {
			return;
		}

		final TopicQosHolder holder = new TopicQosHolder();
		participant.getSALParticipant().get_default_topic_qos(holder);
		...
		topic = participant.getSALParticipant().create_topic(topicName, topicName, topicQos, null, ANY_STATUS.value);
