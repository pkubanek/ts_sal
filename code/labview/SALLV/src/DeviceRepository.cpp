  ...
  catch (SALLV::DevFailed& df)
  {
    SALLV::Except::re_throw_exception(df,
                                      _CPTC("software error"),
			                                _CPTC("binding error: DeviceRepository instanciation failed [SALLV exception caught]"),
			                                _CPTC("DeviceRepository::init_device_repository"));
  }
  catch (...)
  {
  ...
  catch (SALLV::DevFailed & df)
  {
    SALLV::Except::re_throw_exception(df,
                                      _CPTC("object instanciation failed"),
			                                _CPTC("DeviceDescriptor instanciation failed [SALLV exception caught]"),
			                                _CPTC("DeviceRepository::device_desc"));
  }
  catch (...)
  {
...
// DeviceRepository::device_desc
//=============================================================================
DeviceDescriptor * DeviceRepository::device_desc (SALLV::DeviceProxy * _dp, bool _ownership)
    throw (SALLV::DevFailed)
{
  //- critical section
  omni_mutex_lock guard(this->lock_);
