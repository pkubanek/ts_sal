...
int 
Tcl2Ndds_mpUpdateSettings (mpUpdateSettings nddsObj, int argc, char **argv) {
  int count = 0;
  int i;

  if (mpUpdateSettings_ArgCount != argc) {
    fprintf (stderr,"ERROR(tcl2ndds): wrong arg count for mpUpdateSettings.\n");
    fprintf (stderr,"                 got %d, expected %d\n",argc,mpUpdateSettings_ArgCount);
    return (-1);
  }
  ...
  sscanf  (argv[count++], "%f", &nddsObj->angularTolerance);
...
} /* mpUpdateSettingsStruct */


int 
Ndds2Tcl_mpUpdateSettings (mpUpdateSettings nddsObj, Tcl_Interp *interp) {
  char tmpStr[256];
  int  i;
  char *newValue;
  ...
  sprintf (tmpStr, "%f", nddsObj->angularTolerance);  newValue = Tcl_SetVar2 (interp, "this", "angularTolerance", tmpStr, TCL_LEAVE_ERR_MSG);
