  ...
  for (unsigned int i = 0; i < nerr; i++)
  {
    switch (i)
    {
      case 0:
        ...
        break;
      case 1:
        str = "unknown error (may be due to a bug in the SALLV binding for Labview)";
        break;
      case 2:
...
//=============================================================================
int ErrorStack::devfailed (const SALLV::DevFailed& _SALLV_error, LvStringArrayHdl& lv_error_)
{
  unsigned long nerr = _SALLV_error.errors.length();
  ...
  {
    //- severity ---------------------------------------------------------------
    switch(_SALLV_error.errors[i].severity)
    {
      case SALLV::WARN:
        severity = "WARNING";
        break;
    ...
    //- reason ------------------------------------------------------------------
    sh = 0;
    l = ::strlen(_SALLV_error.errors[i].reason);
    err = ::NumericArrayResize(iB, 1L, reinterpret_cast<UHandle*>(&sh), l);
    if (err != noErr) {
      return kError;
    }
	  (*sh)->length = l;
    ::memcpy((*sh)->data, _SALLV_error.errors[i].reason, l);
    (*lv_error_)->data[j++] = sh;
    //- desc --------------------------------------------------------------------
    sh = 0;
    l = ::strlen(_SALLV_error.errors[i].desc);
    err = ::NumericArrayResize(iB, 1L, reinterpret_cast<UHandle*>(&sh), l);
    if (err != noErr) {
      return kError;
    }
	  (*sh)->length = l;
    ::memcpy((*sh)->data, _SALLV_error.errors[i].desc, l);
    (*lv_error_)->data[j++] = sh;
    //- origin --------------------------------------------------------------------
    sh = 0;
    l = ::strlen(_SALLV_error.errors[i].origin);
    err = ::NumericArrayResize(iB, 1L, reinterpret_cast<UHandle*>(&sh), l);
    if (err != noErr) {
      return kError;
    }
	  (*sh)->length = l;
    ::memcpy((*sh)->data, _SALLV_error.errors[i].origin, l);
    (*lv_error_)->data[j++] = sh;
  }
  return kNoError;
}
