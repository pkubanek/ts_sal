...
foreach t [lsort $topics] {
  ...
  if { [lindex [split $id _] 1] != "command" } {
      ...
      label .w[set subsys].t[set id] -width 16 -bg LightBlue -textvariable NROW($id)
      place .w[set subsys].l[set id] -x $ix -y $iy
      place .w[set subsys].t[set id] -x [expr $ix + 300] -y $iy
      incr iy 25
      incr n 1
      if { $iy > 525 } {incr ix 400; set iy 0}
  }
}
button .w[set subsys].stop -text "close" -width 150 -command exit -bg orange 
place .w[set subsys].stop -x 10 -y 525
wm geometry .w[set subsys] [expr $ix+40]x550
